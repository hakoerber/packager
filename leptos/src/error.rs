        ...
        match self {
            Self::Database(ref db_error) => match db_error {
                database::Error::Database(_) => (
                    ...
                    ErrorPage::build(&self.to_string()),
                ),
            ...
            Self::Request(request_error) => match request_error {
                ...
                RequestError::RefererInvalid { message } => (
                    StatusCode::BAD_REQUEST,
                    ErrorPage::build(&format!("referer could not be converted: {message}")),
                ),
                RequestError::EmptyFormElement { name } => (
                    StatusCode::UNPROCESSABLE_ENTITY,
                    ErrorPage::build(&format!("empty form element: {name}")),
                ),
                RequestError::NotFound { message } => (
                    StatusCode::NOT_FOUND,
                    ErrorPage::build(&format!("not found: {message}")),
                ),
                RequestError::Auth { inner: e } => (
                    StatusCode::UNAUTHORIZED,
                    ErrorPage::build(&format!("authentication failed: {e}")),
                ),
                RequestError::Transport { inner } => (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    ErrorPage::build(&inner.to_string()),
                ),
            },
